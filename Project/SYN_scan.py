...
def backlog_syn_scan(bcklg_size, zombie_ip, zombie_port, target_ip): # for now it doesn't cycle on the whole subnet
    ...
    ports = np.random.default_rng().choice([x for x in range(1024, 49151)], syn_packets_number, replace=False) # we use a list of random ports since we don't actually care if it's open or not
    ...
    for packet, response in answered: 
        if response.flags == "A": # how many do we need?
            ack += 1. 
            print(response.sport, response.flags)
    print(f"Ack received: {ack}, Total packets: {packets_number}") # maybe if ack > treshold we consider it alive
    
    return (target_ip, ack/packets_number)